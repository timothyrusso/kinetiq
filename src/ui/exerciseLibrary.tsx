/**
 * The exercise library: browse, search and filter the remote catalog.
 *
 * Rendered in two places: as the Exercises tab's root, and pushed over an exercise's detail
 * when the user asks for similar exercises or taps a muscle or equipment tag there. Pushed,
 * it sits in the root stack, so the header's back button returns to the exercise the user
 * came from; switching to the tab instead took the detail off the stack with no way back.
 * Both read and write the one filter store, which is what makes the tab show the same
...
export function ExerciseLibrary({
  ...
  /** This list's filter: the tab's, or one a pushed list owns. */
  store: ExerciseFilterStore;
  /** How the filter sheet finds `store`; absent for the tab's. */
  storeKey?: string;
  inTab?: boolean;
}) {
  const { t } = useT();
  ...
  // On the tab, clear the bar and its accessory; pushed, only the home indicator.
  const tabBottom = useTabContentBottom();
  const screenBottom = useScreenContentBottom();
  const bottomSpace = inTab ? tabBottom : screenBottom;

  const { filter } = useExerciseFilter(store);
  const settling = useIsQuerySettling(store);
  ...
  const activeChips = useMemo(
    () =>
      [
        { key: 'category', name: nameOf(taxonomy.data?.categories, filter.categoryId), clear: () => store.setCategoryId(null) },
        { key: 'muscle', name: nameOf(taxonomy.data?.muscles, filter.muscleId), clear: () => store.setMuscleId(null) },
        { key: 'equipment', name: nameOf(taxonomy.data?.equipment, filter.equipmentId), clear: () => store.setEquipmentId(null) },
      ].filter((chip): chip is { key: string; name: string; clear: () => void } => chip.name !== null),
    [filter.categoryId, filter.equipmentId, filter.muscleId, store, taxonomy.data],
  );
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: Exercise; index: number }) => (
      <ExerciseItem
        ...
        first={index === 0}
        muscleIds={muscleIds}
        onMuscle={store.setMuscleId}
        onOpen={openExercise}
      />
    ),
    [muscleIds, openExercise, search.isPlaceholder, store, theme],
  );
  ...
  // What the bar last reported. Android's bar re-announces its text when the screen comes back
  // into view, and a bar that never held the list's query (below) announces "": taken at face
  // value, that wiped the search of a pushed list the moment the user came back to it. A
  // report that repeats the last one is the bar talking about itself, not someone typing.
  const barText = useRef('');
  const onSearchText = useCallback(
    (event: NativeSyntheticEvent<TextInputFocusEventData>) => {
      const text = event.nativeEvent.text;
      if (text === barText.current) return;
      barText.current = text;
      store.setQuery(text);
    },
    [store],
  );
  ...
  // A pushed list can open on a search ("browse similar" searches the exercise's name). The
  // bar keeps its own text, so it is handed the query, or it would sit empty over a list that
  // is plainly filtered: once on mount (iOS, where the bar already exists), and again when
  // the bar is opened (Android, where it is only built then and the first call is dropped).
  const fillSearch = useCallback(() => {
    const { draft: current } = store.getState();
    if (current !== barText.current) searchRef.current?.setText(current);
  }, [store]);
  useEffect(fillSearch, [fillSearch]);
  ...
  const openFilters = useCallback(
    () => router.push(routes.exerciseFilters(storeKey)),
    [router, storeKey],
  );
 ...
 * One catalog row.
 *
 * Memoised with an id-taking `onOpen` and `onMuscle`, so `renderItem` passes the same callback to every row
    ...
    () =>
      exercise.primaryMuscles.map((muscle) => {
        ...
        return id === undefined
          ...
          : { key: `m:${muscle}`, label: muscle, onPress: () => onMuscle(id) };
      }),
    [exercise.primaryMuscles, muscleIds, onMuscle],
  );
